...
<<<<<<< HEAD
    const fetchMetrics = async () => {
      try {
        ...
        setMetrics({
          empathy: Math.round(data.model_accuracies.emotion_accuracy * 100),
          quality: Math.round(data.model_accuracies.ranking_score * 100),
          hallucination: Math.round((1 - data.model_accuracies.safety_accuracy) * 100),
          escalation: Math.round(data.model_accuracies.urgency_accuracy * 100)
        });
        
        setHealthScore(Math.round(data.model_accuracies.safety_accuracy * 100));
        ...
        setLogs(prev => {
          ...
          if (data.interventions > 0) {
            newLogs.push({
              t: new Date().toLocaleTimeString([], { hour12: false }),
              msg: `OVERSIGHT: ${data.interventions} SENSITIVE DATA LEAKAGE ATTEMPTS SECURED`,
              type: "warning"
            });
          }
          newLogs.push({
            t: new Date().toLocaleTimeString([], { hour12: false }),
            msg: `GUARDIAN_HEARTBEAT: Nominals check=OK, system_stress=NOMINAL`,
            type: "system"
          });
          return newLogs.slice(-12);
        });
      } catch (e) {
        console.error("Failed to fetch supervisor metrics:", e);
      }
    };
